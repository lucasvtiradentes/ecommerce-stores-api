...
import { addTimeToDateObject, convertDateInfoObjToStringDate, converteDateToUTC, getDateInfoObjFromIsoDate } from '../libraries/dates'
...
const FROM_COLORS_ARR = [
  ...
  [FROM_ARR[5], clc.bgRedBright.bold, clc.red],
  [FROM_ARR[6], clc.white, clc.black]
]
...
function getDateTimePart(options: loggerOptions) {
  ...
  try {
    ...
    const dateTimeRow = `${CURRENT_DATETIME('time')} ${DEFAULT_SEPARATOR} `
