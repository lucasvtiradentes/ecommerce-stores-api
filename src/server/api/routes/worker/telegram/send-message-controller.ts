...
export default async function telegramMessageController(request: Request, response: Response) {
  ...
  let currentJob = await WORKERQUEUE.add({ method: "/api/telegram/sendmessage", ...request.query });
